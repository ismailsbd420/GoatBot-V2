...
onStart: async function ({ api, args, message, event, threadsData, usersData, dashBoardData, globalData, threadModel, userModel, dashBoardModel, globalModel, role, commandName, getLang }) {
    const qaFile = './scripts/cmds/bypass_json/bypass.json';
    const qna = JSON.parse(fs.readFileSync(qaFile));

    const newQaFile = './scripts/cmds/bypass_json/new_bypass.json';
    let newQna = {};
  ...
  if (!botAnswer) {
      try {
        const response = await axios.get(`https://api.samir-dev.repl.co/gpt/ask?q=${encodeURIComponent(userQuestion)}`);
